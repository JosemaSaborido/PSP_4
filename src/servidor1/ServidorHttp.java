 ...
 * Devolverá un saludo al nombre y apellido que se pase por parametros GET
 * @author Jose Manuel Saborido Noriega
 */
public class ServidorHttp {
    //ATRIBUTOS
    private static final int PUERTO_DEFAULT = 80;
    private static final String PATH = "/saludar";
    ...
    private static boolean crearServidor(Integer puerto){
        try {
            ...
            System.out.println("Creando instanacia de servidor en el puerto " 
                    + puerto + "...");
            sv = HttpServer.create(new InetSocketAddress(puerto), 0);
            sv.createContext(PATH, new HttpHandlerSaludar());
            //sv.setExecutor(null);
            sv.start();
